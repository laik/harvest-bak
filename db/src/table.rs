...
impl<'a, K, V> Accessor<'a, K, V> {
    ...
    pub fn get(&self) -> &'a V {
        ...
        unsafe { &*self.table.values.offset(self.idx as isize) }
    }
}
...
impl<'a, K, V> MutAccessor<'a, K, V> {
    ...
    pub fn get(&mut self) -> &'a mut V {
        debug_assert!(self.table.is_present(self.idx));
        unsafe { &mut *self.table.values.offset(self.idx as isize) }
    }
}
    ...
    K: Hash + Eq,
{
    pub fn new(capacity: usize) -> Table<K, V> {
        assert!(
            size_of::<K>() > 0 && size_of::<V>() > 0,
            "zero-size types not yet supported"
        );
    ...
    pub fn lookup<C>(&self, hash: u64, eq: C) -> Option<usize>
    where
        C: Fn(&K) -> bool,
    {
        ...
        loop {
            ...
            loop {
                if !self.is_present(i) {
                    unsafe {
                        self.put_at_empty(i, key, value, hash);
                    }
                    self.len += 1;
                    return None;
    ...
    pub fn remove<C>(&mut self, hash: u64, eq: C) -> Option<V>
    where
        C: Fn(&K) -> bool,
    {
    ...
    #[inline]
    fn compare_key_at<C>(&self, eq: &C, idx: usize) -> bool
    where
        C: Fn(&K) -> bool,
    {
    ...
    fn resize(&mut self) {
        ...
        unsafe {
            self.foreach_present_idx(|i| {
                ...
                new_table.put(
                    ptr::read(self.keys.offset(i as isize)),
...
impl<K, V> Table<K, V> {
    ...
    pub fn clear(&mut self) {
        self.foreach_present_idx(|i| unsafe {
            drop_in_place::<K>(self.keys.offset(i as isize));
    ...
    fn foreach_present_idx<F>(&self, mut f: F)
    where
        F: FnMut(usize),
    {
        let mut seen = 0;
