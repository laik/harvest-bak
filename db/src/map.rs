...
pub struct ConcHashMap<K, V, H = RandomState>
where
    K: Send + Sync,
    V: Send + Sync,
{
    ...
    H: BuildHasher,
{
    ...
    pub fn with_options(opts: Options<H>) -> ConcHashMap<K, V, H> {
        ...
        let partitions = conc
            ...
            .unwrap_or((conc / 2).next_power_of_two());
        ...
        ConcHashMap {
            ...
            table_shift: if partitions == 1 {
                0
            } else {
                64 - partitions.trailing_zeros() as u64
            },
    ...
    pub fn insert(&self, key: K, value: V) -> Option<V> {
        ...
        table.put(key, value, hash, |old, mut new| {
            swap(old, &mut new);
            new
        })
    }
    ...
    pub fn upsert<U: Fn(&mut V)>(&self, key: K, value: V, updater: &U) {
        ...
        let table_idx = self.table_for(hash);
        let mut table = self.tables[table_idx].lock();
        table.put(key, value, hash, |old, _| {
            updater(old);
        });
    }
...
impl<'a, K, V, H> Iterator for Entries<'a, K, V, H>
where
    K: Send + Sync,
    V: Send + Sync,
{
    ...
    fn next(&mut self) -> Option<(&'a K, &'a V)> {
        loop {
            ...
            let res: Option<(&'a K, &'a V)> =
                unsafe { ::std::mem::transmute(self.table.iter_advance(&mut self.bucket)) };
            match res {
...
mod test {
    ...
    fn mut_modify() {
        ...
        map.insert(1, 0);
        let e = map.find_mut(&1).unwrap().get();
        *e += 1;
    ...
    fn conc_mut_modify() {
        ...
        let writer = thread::spawn(move || {
            for i in 0..range {
                let e = tl_map.find_mut(&i).unwrap().get();
                *e += 1;
            }
        });
